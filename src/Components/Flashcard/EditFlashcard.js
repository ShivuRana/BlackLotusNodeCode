...
const EditFlashcard = () => {
    ...
    const selecthandleOnChange = (selectlistValue, name, setvalue, i, l) => {
        console.log(selectlistValue, name, setvalue, "selectlistValue");
        setModuleId(setvalue)
        setTopicId(setvalue)
        setFlashCardInput({ ...FlashCardInput, [name]: selectlistValue })
        // setFlashCardInput({ ...FlashCardInput, [name]: setvalue })
        ...
        setError(showerror)
        console.log(FlashCardInput, "2154")
    ...
    console.log(FlashCardInput)
    // console.log(FlashCardInput.moduleId)
    ...
    const submitAddFlashcard = (e) => {
        ...
        if (FlashCardInput.moduleName !== "" && FlashCardInput.topicName !== "" && FlashCardInput.question) {
            ...
            var data = {
                ...
                moduleId: FlashCardInput.moduleName,
                topicId: FlashCardInput.topicName,
                questionId: null
            }
    ...
    useEffect(() => {
        ...
        console.log(displayApiInformation, "mayur")
        authService.GetFlashcardByID(location.state).then((response) => {
            ...
            console.log(FlashCardInput.moduleId, "*******************")
            // var data = FlashCardInput.moduleId !== undefined && FlashCardInput.moduleId

            // authService.GetAllTopicsById(data).then((response) => {
            //     if (response.data.status) {
            //         var newGetEditedData1 = {
            //             topicName: response.data.data.name,
            //             topicId: response.data.data._id,
            //         }
            //         setFlashCardInput(newGetEditedData1)
            ...
            //     console.log(e)
            // })
            setFlashCardInput(newGetEditedData)
        ...
        authService.GetModuleListing(data).then((response) => {
            if (response.data.status) {
                setMonthTypeData(response.data.data)
            }
        }).catch((e) => {
            console.log(e)
        })

        // AuthService.getModuleById(response.data.data.moduleId).then((response) => {
        authService.getModuleById("633a8b47149e8883964c264c").then((response) => {
            if (response.data.status) {
                console.log(response.data.data.name, "view")
                var newGetEditedData = {
                    moduleName: response.data.data.name,
                    moduleId: response.data.data._id,
                }
                setFlashCardInput(newGetEditedData)
                //for topics

                console.log(response.data.data._id, "FlashCardInput._id")
                // authService.GetAllTopicsById(response.data.data._id).then((response) => {
                //     if (response.data.status) {
                //         const numAscending = response.data.data.sort((a, b) => a.order - b.order);
                //         setTopicTypeData(numAscending)
                //         console.log(numAscending, "topics is")
                //     }
                // }).catch((e) => {
                //     console.log(e)
                // })
                            ...
                            <div className='cstm-navigation'>
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">Edit flashcard</li>
                                    </ol>
                                </nav>
                            </div>
                        </div>
                            ...
                            <div className='addtopic-main'>
                                <form>
                                    <div className='addtopic-field'>
                                        ...
                                        <div className='col-6 InputField-1 cstm-ModalField '>
                                            ...
                                            <Select
                                                ...
                                                value={{ value: FlashCardInput.moduleName, label: FlashCardInput.moduleName }}
                                                onChange={(e) => { selecthandleOnChange(e.value, 'moduleName', e.setvalue); }}
                                                searchable
                                            />
                                        ...
                                        <div className='col-6 InputField-1'>
                                            ...
                                            <div class="dropdown">
                                                <Select
                                                    ...
                                                    value={{ value: FlashCardInput.topicName, label: FlashCardInput.topicName }}
                                                    onChange={(e) => { selecthandleOnChange(e.value, 'topicName', e.setvalue); }}
                                                    searchable
                                                />
                                    ...
                                    <div class="col-12 modal-addbtn">
                                        <button type='button' class="cstm-btn1 cstm-updatebtn" data-toggle="modal" data-target="#FlashcardCreate" onClick={submitAddFlashcard}>Update</button>
                                    </div>
                                </form>
                            </div>
                        </div>
